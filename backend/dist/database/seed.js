...
const seed_attendance_1 = require("./seed-attendance");
const seed_leave_types_1 = require("./seed-leave-types");
const seed_company_holidays_1 = require("./seed-company-holidays");
...
const slugifyDept = (name) => name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
...
const runSeed = async () => {
    ...
    await (0, seed_leave_types_1.runSeedLeaveTypes)();
    await (0, seed_company_holidays_1.runSeedCompanyHolidays)();
    ...
    const [adminHash, gateHash, managerHash, employeeHash] = await Promise.all([
        bcryptjs_1.default.hash('Admin@123', SALT_ROUNDS),
        bcryptjs_1.default.hash('Gate@123', SALT_ROUNDS),
        bcryptjs_1.default.hash('Manager@123', SALT_ROUNDS),
        bcryptjs_1.default.hash('Emp@123', SALT_ROUNDS),
    ]);
    // ── Admin (1) ──────────────────────────────────────────────────────────────
    await pool.query(`INSERT INTO users (name, email, password, role_id, department_id)
     VALUES ($1, $2, $3, $4, $5)`, ['Sarah Johnson', 'admin@company.com', adminHash, roleIdMap.admin, deptIdMap['Software R&D']]);
    console.log('  ✅ Seeded admin: admin@company.com / Admin@123');
    // ── Gatekeeper (1) ─────────────────────────────────────────────────────────
    await pool.query(`INSERT INTO users (name, email, password, role_id, department_id)
     VALUES ($1, $2, $3, $4, $5)`, ['Mike Carter', 'gatekeeper@company.com', gateHash, roleIdMap.gatekeeper, deptIdMap.Store]);
    console.log('  ✅ Seeded gatekeeper: gatekeeper@company.com / Gate@123');
    // ── Guest (1) ──────────────────────────────────────────────────────────────
    const guestHash = await bcryptjs_1.default.hash('Guest@123', SALT_ROUNDS);
    await pool.query(`INSERT INTO users (name, email, password, role_id, department_id)
     VALUES ($1, $2, $3, $4, $5)`, ['Guest User', 'guest@company.com', guestHash, roleIdMap.guest, deptIdMap['Software R&D']]);
    console.log('  ✅ Seeded guest: guest@company.com (also via guest code login)');
    // ── Managers (1 per department) ────────────────────────────────────────────
    const managerEmailByDept = {};
    for (const [index, dept] of seedDepartments.entries()) {
        ...
        await pool.query(`INSERT INTO users (name, email, password, role_id, department_id)
       VALUES ($1, $2, $3, $4, $5)`, [name, email, managerHash, roleIdMap.manager, deptIdMap[dept]]);
        managerEmailByDept[dept] = email;
        console.log(`  ✅ Seeded manager (${dept}): ${email} / Manager@123`);
    }
    const managerRows = await pool.query(`SELECT u.id, u.email, d.name AS department
     FROM users u
     JOIN roles r ON r.id = u.role_id
     JOIN departments d ON d.id = u.department_id
     WHERE r.name = 'manager'`);
    const managerIdByDept = {};
    ...
    for (let i = 1; i <= EMPLOYEE_COUNT; i += 1) {
        ...
        placeholders.push(`($${param++}, $${param++}, $${param++}, $${param++}, $${param++}, $${param++})`);
        employeeValues.push(name, email, employeeHash, roleIdMap.employee, deptIdMap[dept], managerId);
    }
    await pool.query(`INSERT INTO users (name, email, password, role_id, department_id, manager_id)
     VALUES ${placeholders.join(', ')}`, employeeValues);
    console.log(`  ✅ Seeded ${EMPLOYEE_COUNT} employees (password: Emp@123)`);
    console.log('     Examples: emp001@company.com … emp150@company.com');
    const counts = await pool.query(`SELECT r.name AS role, COUNT(u.id)::int AS user_count
     FROM roles r
    ...
    await pool.query(`UPDATE users
     SET leave_balance = $1,
         leave_accrued_through = $2`, [leaveBalance, accruedThrough]);
    ...
    await (0, seed_attendance_1.seedAttendanceLast3Months)();
};
