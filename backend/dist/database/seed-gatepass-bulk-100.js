...
const run = async () => {
    ...
    `);
        ...
        await client.query(`DELETE FROM gatepass_approval_requests
       WHERE gatepass_id IN (
        ...
        for (let i = 1; i <= 100; i += 1) {
         ...
         RETURNING id`, [
            ...
            if (scenario.approvalFlow === 'manager_then_admin') {
                const managerStatus = scenario.status === 'pending_manager_approval'
                    ? 'pending'
                    : scenario.status === 'pending_admin_approval'
                        ...
                        : ['rejected', 'cancelled'].includes(scenario.status)
                            ? 'cancelled'
                            : 'approved';
                await client.query(`INSERT INTO gatepass_approval_requests (gatepass_id, approver_user_id, approver_role, step, status, acted_at)
           ...
           ON CONFLICT (gatepass_id, step) DO NOTHING`, [
            ...
            const adminStatus = scenario.status === 'pending_admin_approval' || scenario.status === 'pending_manager_approval'
                ? 'pending'
                : scenario.status === 'rejected'
                    ...
                    : scenario.status === 'cancelled'
                        ? 'cancelled'
                        : 'approved';
            await client.query(`INSERT INTO gatepass_approval_requests (gatepass_id, approver_user_id, approver_role, step, status, remarks, acted_at)
        ...
        const count = await pool.query(`SELECT COUNT(*)::int AS count FROM gatepasses WHERE reason_description LIKE $1`, [`${BULK_TAG}%`]);
...
//# sourceMappingURL=seed-gatepass-bulk-100.js.map